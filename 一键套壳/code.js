...
jsDesign.ui.onmessage = msg => {
  if (msg.type === 'event:done') {
    ...
    // const scale = 1
    const [, , scale] = scaleImage(modew, modeh, rect.width , rect.height)
    // console.log('scale----', scale)
    
    const width = rect.width - loffset * scale -  roffset * scale
    const height = rect.height - toffset * scale - boffset * scale
    ...
    renderFrame.x = loffset * scale
    renderFrame.y = toffset * scale
